...
export function Marking2Page() {
  ...
  // };
  // const currentStep = "marking2";
  const currentStep = "marking2"; // or derive from login role
  ...
  const handleQuickAssign = (order: AssemblyOrderData) => {
    const currentStep = "marking2"; // 👈 set dynamically based on page
    const nextSteps = getNextSteps(currentStep);
  ...
  const handlePrintBinCard = () => {
    const cards = selectedOrdersData
      ...
      <div style="border:1px solid #ccc; padding:20px; border-radius:10px; margin-bottom:30px; page-break-inside: avoid;">
        ...
        <div style="margin-bottom:15px;"><strong>Item Description:</strong><br><span style="padding-top:2px;">${order.product}</span></div>
...
const handleSaveRemarks = () => {
  ...
  // Update in table orders so UI refreshes
  setOrders(prev =>
    prev.map(o =>
  ...
  // Update in view details dialog if open
  setViewedOrder(prev =>
    prev && prev.id === remarksOrder.id
      ? { ...prev, remarks: remarksText }
      : prev
  );
