...
export function SvsPage() {
  ...
  const handleQuickAssign = (order: AssemblyOrderData) => {
    ...
    const nextStep =
      order.assemblyLine?.toLowerCase() === "d" ? "marking2" : "marking1";
  ...
  const handleAssignOrder = async () => {
    ...
    try {
      ...
      const nextStepKey =
        quickAssignStep || (Array.isArray(nextSteps) ? nextSteps[0] : "svs");
          ...
          <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-6">
            <div className="flex-row-main">
              <h1 className="text-gray-900 mb-2 text-2xl font-semibold">
                Stock Valve Store (SVS) Management
              </h1>
              <p className="text-sm text-gray-600">
                Track and manage stock valve store orders and manufacturing
                workflow (Finished Valve = Yes)
              </p>
            </div>
