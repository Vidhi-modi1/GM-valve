...
export function PlanningPage() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      // Accept both string "true" or boolean-like "RCS" responses — adapt per your backend
      const ok = res?.data?.Resp_code === 'true' || res?.data?.Resp_code === 'RCS';
      if (ok && Array.isArray(res.data.data)) {
        const apiOrders: AssemblyOrderData[] = res.data.data.map((item: any) => ({
          ...
          // ✅ Preserve urgent status from backend
          alertStatus:
            item.is_urgent === true ||
            item.is_urgent === "true" ||
...
// ✅ Marks urgent only, no toggle behavior
const toggleAlertStatus = async (orderId: string) => {
  try {
    ...
    if (order.alertStatus === true || order.alertStatus === "true" || order.urgent === 1) {
    ...
    console.log("Payload sent:", Object.fromEntries(fd.entries()));

    const res = await axios.post(`${API_URL}/mark-urgent`, fd, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
    ...
    if (res.data?.Resp_code === "true") {
      // ✅ Update local UI state only one way (mark red)
      setOrders((prev) =>
        prev.map((o) =>
          o.id === orderId ? { ...o, alertStatus: true } : o
        )
      );

      setMessage("🔴 Order marked as urgent");
      // optional refresh, but ensure it doesn’t override UI too early
      setTimeout(fetchOrders, 1500);
    } else {
      setMessage(res.data?.Resp_desc || "Failed to mark urgent");
    }
  } catch (err: any) {
    console.error("Error marking urgent:", err);
    setMessage("❌ Error marking urgent");
  } finally {
                  ...
                  <tr key={order.id} className="group hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2 text-center text-sm text-gray-900">
                      <Button
                        size="sm"
                        variant="ghost"
                        className={`h-7 w-7 p-0 ${getRemark(order.id) ? 'bg-[#174a9f] hover:bg-[#123a7f]' : 'hover:bg-[#d1e2f3]'}`}
                        title="Add/Edit Remarks"
                        onClick={() => handleOpenRemarks(order)}
                      >
                        <MessageSquarePlus className={`h-4 w-4 ${getRemark(order.id) ? 'text-white' : 'text-blue-600'}`} />
                      </Button>
                    </td>
  ...
  onClick={() => {
    // only call API if not already urgent
    if (!getAlertStatus(order.id)) toggleAlertStatus(order.id);
  }}
>
  <Siren
    className={`h-4 w-4 ${
      ...
      <Dialog open={quickAssignOpen} onOpenChange={setQuickAssignOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>Assign {selectedOrder?.uniqueCode} to the next workflow step</DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-6 py-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input id="assignQty" type="number" value={quickAssignQty} onChange={(e) => setQuickAssignQty(e.target.value)} max={selectedOrder?.qtyPending} />
              </div>
            </div>

            <div className="text-sm text-gray-500">Available Quantity: <span className="font-medium text-gray-900">{selectedOrder?.qtyPending}</span></div>

            <div className="space-y-2 border-t pt-4">
              <div className="flex items-center gap-2">
                <Checkbox id="splitOrder" checked={splitOrder} onCheckedChange={(val) => setSplitOrder(Boolean(val))} />
                <Label htmlFor="splitOrder">Split order to multiple workflow steps</Label>
              </div>
                ...
                <div className="pl-4 border-l-2 border-blue-100 space-y-3">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Input type="number" value={splitAssignQty} onChange={(e) => setSplitAssignQty(e.target.value)} max={selectedOrder?.qtyPending} />
                    </div>
                  </div>

                  {quickAssignErrors.sameEngineer && <div className="p-3 bg-red-50 text-red-600 rounded">{quickAssignErrors.sameEngineer}</div>}
                  {quickAssignErrors.totalQtyMismatch && <div className="p-3 bg-amber-50 text-amber-700 rounded">{quickAssignErrors.totalQtyMismatch}</div>}
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-gray-100">
            ...
            <Button onClick={handleQuickAssignSave} className="bg-black text-white">Assign</Button>
          </div>
        </DialogContent>
      </Dialog>
