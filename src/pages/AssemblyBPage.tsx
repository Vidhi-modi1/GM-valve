...
totalQty: Number(item.totalQty || item.total_qty || item.qty || 0), 
  ...
  const currentStep = "assembly-b"; // or derive from login role
  ...
  //   const currentStep = "assembly"; // 👈 set dynamically based on page
  //   const nextSteps = getNextSteps(currentStep);
  ...
  //   const isD = (order.assemblyLine || '').toUpperCase().includes('D');
...
const handleAssignOrder = async () => {
  ...
  try {
    ...
    // ✅ Determine next step (workflow or dropdown)
    const nextStepKey =
      quickAssignStep ||
      (Array.isArray(nextSteps) ? nextSteps[0] : "");

    // ✅ Convert step keys → backend readable labels
