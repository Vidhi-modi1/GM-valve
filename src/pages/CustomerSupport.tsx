...
function CustomerSupport() {
  ...
  const [selectedAssemblyLine, setSelectedAssemblyLine] =
    useState<string>("all");
  ...
  const reloadData = async () => {
    ...
    try {
      ...
      const res = await axios.post(CUSTOMER_SUPPORT_ENDPOINT, payload, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const saveDeliveryDate = async (order: OrderData, date: string) => {
    try {
      ...
      const res = await axios.post(`${API_URL}/add-delivery-date`, form, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const markingOrders = [
    ...(dataByStage["marking1"] || []),
    ...(dataByStage["marking2"] || []),
  ];
  ...
  const stats = useMemo(() => {
    ...
    const totalQty =
      apiCounts?.totalQty ??
      allOrders.reduce((sum, order) => sum + (order.totalQty || 0), 0);
    const totalCompleted =
      apiCounts?.completeOrders ??
      allOrders.reduce((sum, order) => sum + order.qtyExe, 0);
    const totalPending =
      apiCounts?.pendingQty ??
      allOrders.reduce((sum, order) => sum + order.qtyPending, 0);
    const alertCount =
      apiCounts?.urgentOrders ??
      allOrders.filter((order) => order.alertStatus).length;

    // Calculate pending quantities for each stage
    const materialIssuePending = materialIssueOrders.reduce(
      (sum, order) => sum + order.qtyPending,
      0
    );
    const semiQcPending = semiQcOrders.reduce(
      (sum, order) => sum + order.qtyPending,
      0
    );
    const phosphatingQcPending = phosphatingOrders.reduce(
      (sum, order) => sum + order.qtyPending,
      0
    );
    const assemblyPending = assemblyOrders.reduce(
      (sum, order) => sum + order.qtyPending,
      0
    );
    const assemblyAPending = assemblyAOrders.reduce(
      (sum, order) => sum + order.qtyPending,
      0
    );
    const assemblyBPending = assemblyBOrders.reduce(
      (sum, order) => sum + order.qtyPending,
      0
    );
    const assemblyCPending = assemblyCOrders.reduce(
      (sum, order) => sum + order.qtyPending,
      0
    );
    const assemblyDPending = assemblyDOrders.reduce(
      (sum, order) => sum + order.qtyPending,
      0
    );
    const testingPending = testing1Orders.reduce(
      (sum, order) => sum + order.qtyPending,
      0
    );
    const testingPending2 = testing2Orders.reduce(
      (sum, order) => sum + order.qtyPending,
      0
    );
    const svsPending = svsOrders.reduce(
      (sum, order) => sum + order.qtyPending,
      0
    );
    const markingPending = markingOrders.reduce(
      (sum, order) => sum + order.qtyPending,
      0
    );
    const testing1Pending = (dataByStage["testing1"] || []).reduce(
      (sum, order) => sum + order.qtyPending,
      0
    );
    const testing2Pending = (dataByStage["testing2"] || []).reduce(
      (sum, order) => sum + order.qtyPending,
      0
    );
    const marking1Pending = (dataByStage["marking1"] || []).reduce(
      (sum, order) => sum + order.qtyPending,
      0
    );
    const marking2Pending = (dataByStage["marking2"] || []).reduce(
      (sum, order) => sum + order.qtyPending,
      0
    );
    const pdi1Pending = pdi1Orders.reduce(
      (sum, order) => sum + order.qtyPending,
      0
    );
    const pdi2Pending = pdi2Orders.reduce(
      (sum, order) => sum + order.qtyPending,
      0
    );
    const tpiPending = tpiOrders.reduce(
      (sum, order) => sum + order.qtyPending,
      0
    );
    const dispatchPending = dispatchOrders.reduce(
      (sum, order) => sum + order.qtyPending,
      0
    );
    ...
    const makeCounts = (orders: OrderData[]) => ({
      inProgress: orders.filter((o) => o.qtyExe > 0 && o.qtyPending > 0).length,
        ...
        (o) => o.qtyPending === 0 && (o.totalQty > 0 || o.qtyExe > 0)
      ).length,
    });
      ...
      .then(async (ExcelJSModule) => {
        ...
        headerRow.eachCell((cell, colNumber) => {
          cell.fill = {
            ...
            pattern: "solid",
            fgColor: {
              argb: headerColors[colNumber]
                ? "FF" + headerColors[colNumber]
                : "FFF3F4F6",
            },
          };
          cell.border = {
        ...
        filteredOrders.forEach((order) => {
          worksheet.addRow({
            ...
            materialIssue: getStageStatus(
              order,
              getStepLabel("material-issue")
            ),
  ...
  const getProgressPercentage = (order: OrderData) => {
    return order.totalQty > 0
      ? Math.round((order.qtyExe / order.totalQty) * 100)
      : 0;
  };
  ...
  const getStageStatus = (
    order: OrderData & { currentStage: string },
    stage: string
  ) => {
    ...
    if (v != null) {
      ...
      if (vNorm === "in_progress" || vNorm === "inprogress")
        return "IN PROCESS";
    ...
    const stageIndex = order.workflowHistory.findIndex(
      (w) => normalizeForMatch(w.stage) === target
    );
  ...
  return (
    <>
      ...
      <div className="p-6 max-w-[1920px] mx-auto space-y-6 animate-fade-in-up">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Comprehensive view of all orders across all workflow stages
              </p>
            </div>
            <Button
              onClick={reloadData}
        ...
        <Card className="p-4 bg-white/70 backdrop-blur-sm border-gray-200">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <Select
              ...
              onValueChange={setSelectedAssemblyLine}
            >
              <SelectTrigger className="w-full md:w-[200px] border-gray-300">
        ...
        {/* Orders Table */}
        <Card
          className="bg-white/70 backdrop-blur-sm border-gray-200 overflow-hidden"
          style={{
            ...
            <table className="w-full">
              <thead className="table-head sticky top-16 z-30 bg-white">
                <tr className="bg-gradient-to-r from-[#174a9f]/10 to-indigo-50/50 border-b border-gray-200">
                  <th className="px-4 py-3 text-center text-gray-700 sticky left-0 bg-gradient-to-r from-[#174a9f]/10 to-indigo-50/50 z-10 ctm-opacity">
                    Assembly Line
                  </th>
                  <th className="px-4 py-3 text-gray-700 truncate text-center">
                    SOA NO.
                  </th>
                  <th className="px-4 py-3 text-gray-700 text-center truncate">
                    SR.NO.
                  </th>
                  <th className="px-4 py-3 text-center text-gray-700 truncate text-center">
                    Assembly Date
                  </th>
                  <th className="px-4 py-3 text-gray-700 truncate text-center">
                    Unique Code
                  ...
                  <th className="px-4 py-3 text-center text-gray-700 bg-sky-50/80 truncate text-center">
                  ...
                  <th className="px-4 py-3 text-center text-gray-700 bg-blue-50/80">
                    Planning
                  </th>
                  <th className="px-4 py-3 text-center text-gray-700 bg-purple-50/80">
                    Material Issue
                  </th>
                  <th className="px-4 py-3 text-center text-gray-700 bg-yellow-50/80">
                    Semi QC
                  </th>
                  <th className="px-4 py-3 text-center text-gray-700 bg-orange-50/80">
                    Phosphating QC
                  </th>
                  <th className="px-4 py-3 text-center text-gray-700 bg-indigo-50/80">
                    Assembly A
                  </th>
                  <th className="px-4 py-3 text-center text-gray-700 bg-indigo-50/80">
                    Assembly B
                  ...
                  <th className="px-4 py-3 text-center text-gray-700 bg-pink-50/80">
                    Testing1
                  </th>
                  <th className="px-4 py-3 text-center text-gray-700 bg-pink-50/80">
                    Testing2
                  </th>
                  <th className="px-4 py-3 text-center text-gray-700 bg-green-50/80">
                    SVS
                  </th>
                  <th className="px-4 py-3 text-center text-gray-700 bg-teal-50/80">
                    Marking1
                  </th>
                  <th className="px-4 py-3 text-center text-gray-700 bg-teal-50/80">
                    Marking2
                  </th>
                  <th className="px-4 py-3 text-center text-gray-700 bg-gray-50/80">
                    PDI1
                  </th>
                  <th className="px-4 py-3 text-center text-gray-700 bg-gray-50/80">
                    PDI2
                  </th>
                  <th className="px-4 py-3 text-center text-gray-700 bg-rose-50/80">
                    TPI
                  </th>
                  <th className="px-4 py-3 text-center text-gray-700 bg-emerald-50/80">
                    Dispatch
                  </th>
                  <th className="px-4 py-3 text-center text-gray-700">
                    Actions
                ...
                {loading ? (
                  ...
                  paginatedOrders.map((order) => (
                    ...
                    >
                      ...
                      <td
                        className="px-4 py-3 text-center text-gray-700 max-w-[300px] truncate"
                        title={order.product}
                      >
                        {order.product}
                      </td>
                      <td className="px-4 py-3 text-center">{order.poQty}</td>
                      ...
                      <td className="px-4 py-3 bg-blue-50/30 truncate">
                        {renderStageCell(
                          getStageStatus(order, getStepLabel("planning"))
                        )}
                      </td>
                      <td className="px-4 py-3 bg-purple-50/30 truncate">
                        {renderStageCell(
                          getStageStatus(order, getStepLabel("material-issue"))
                        )}
                      </td>
                      <td className="px-4 py-3 bg-yellow-50/30 truncate">
                        {renderStageCell(
                          getStageStatus(order, getStepLabel("semi-qc"))
                        )}
                      </td>
                      <td className="px-4 py-3 bg-orange-50/30 truncate">
                        {renderStageCell(
                          getStageStatus(order, getStepLabel("phosphating"))
                        )}
                      </td>
                      <td className="px-4 py-3 bg-indigo-50/30 truncate">
                        {renderStageCell(
                          getStageStatus(order, getStepLabel("assembly-a"))
                        )}
                      </td>
                      <td className="px-4 py-3 bg-indigo-50/30 truncate">
                        {renderStageCell(
                          getStageStatus(order, getStepLabel("assembly-b"))
                        )}
                      </td>
                      <td className="px-4 py-3 bg-indigo-50/30 truncate">
                        {renderStageCell(
                          getStageStatus(order, getStepLabel("assembly-c"))
                        )}
                      </td>
                      <td className="px-4 py-3 bg-indigo-50/30 truncate">
                        {renderStageCell(
                          getStageStatus(order, getStepLabel("assembly-d"))
                        )}
                      </td>
                      <td className="px-4 py-3 bg-pink-50/30 truncate">
                        {renderStageCell(
                          getStageStatus(order, getStepLabel("testing1"))
                        )}
                      </td>
                      <td className="px-4 py-3 bg-pink-50/30 truncate">
                        {renderStageCell(
                          getStageStatus(order, getStepLabel("testing2"))
                        )}
                      </td>
                      <td className="px-4 py-3 bg-green-50/30 truncate">
                        {renderStageCell(
                          getStageStatus(order, getStepLabel("svs"))
                        )}
                      </td>
                      <td className="px-4 py-3 bg-teal-50/30 truncate">
                        {renderStageCell(
                          getStageStatus(order, getStepLabel("marking1"))
                        )}
                      </td>
                      <td className="px-4 py-3 bg-teal-50/30 truncate">
                        {renderStageCell(
                          getStageStatus(order, getStepLabel("marking2"))
                        )}
                      </td>
                      <td className="px-4 py-3 bg-gray-50/30 truncate">
                        {renderStageCell(
                          getStageStatus(order, getStepLabel("pdi1"))
                        )}
                      </td>
                      <td className="px-4 py-3 bg-gray-50/30 truncate">
                        {renderStageCell(
                          getStageStatus(order, getStepLabel("pdi2"))
                        )}
                      </td>
                      <td className="px-4 py-3 bg-rose-50/30 truncate">
                        {renderStageCell(
                          getStageStatus(order, getStepLabel("tpi"))
                        )}
                      </td>
                      <td className="px-4 py-3 bg-emerald-50/30 truncate">
                        {renderStageCell(
                          getStageStatus(order, getStepLabel("dispatch"))
                        )}
                      </td>
