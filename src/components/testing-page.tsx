...
const mockTestingOrders: TestingOrderData[] = [
  {
    id: '1',
    assemblyLine: 'A',
    gmsoaNo: 'SOA0776',
...
export function TestingPage({ searchTerm = '', onSearchChange, onNavigateToOrders, showAddOrderModal = false, setShowAddOrderModal }: OrdersPageProps) {

  const [localSearchTerm, setLocalSearchTerm] = useState('');
  const [quickAssignOpen, setQuickAssignOpen] = useState(false);
  const [selectedOrder, setSelectedOrder] = useState<TestingOrderData | null>(null);
  ...
  const [viewedOrder, setViewedOrder] = useState<TestingOrderData | null>(null);
  const [remarksDialogOpen, setRemarksDialogOpen] = useState(false);
  const [remarksOrder, setRemarksOrder] = useState<TestingOrderData | null>(null);
  ...
  // Quick assign state
  const [quickAssignStep, setQuickAssignStep] = useState('svs');
  ...
  const selectedOrdersData = mockTestingOrders.filter(order => selectedRows.has(order.id));
  ...
  // View order details
  const handleViewDetails = (order: TestingOrderData) => {
    setViewedOrder(order);
    setViewDetailsDialogOpen(true);
  };

  // Open remarks dialog
  const handleOpenRemarks = (order: TestingOrderData) => {
  ...
  const handleSaveRemarks = () => {
    if (remarksOrder) {
      ...
      const orderIndex = mockTestingOrders.findIndex(o => o.id === remarksOrder.id);
      if (orderIndex !== -1) {
        mockTestingOrders[orderIndex].remarks = remarksText;
      }
  ...
  const handleQuickAssign = (order: TestingOrderData) => {
    setSelectedOrder(order);
    // Always set to SVS as the next step after Testing
    setQuickAssignStep('svs');
    ...
    <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in bg-white min-h-screen">
      <div className="mb-8">
        <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-6">
          <div>
            <h1 className="text-gray-900 mb-2">Testing</h1>
            <p className="text-gray-600">Track and manage testing workflow and quality control</p>
          </div>
