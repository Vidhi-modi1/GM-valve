...
export function AfterPhosphatingQcPage({ searchTerm = '', onSearchChange, onNavigateToOrders, showAddOrderModal = false, setShowAddOrderModal }: OrdersPageProps) {
  ...
  // Filter orders based on search term (memoized for performance)
  const filteredOrders = useMemo(() => {
    if (!effectiveSearchTerm.trim()) return mockAssemblyOrders;
    
    const lowerSearchTerm = effectiveSearchTerm.toLowerCase();
    return mockAssemblyOrders.filter(order =>
    ...
    <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in bg-white min-h-screen">
      <div className="mb-8">
        <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-6">
          <div>
            <h1 className="text-gray-900 mb-2">After Phosphating QC</h1>
                    ...
                    <td className="sticky right-0 z-10 bg-white group-hover:bg-gray-50 px-3 py-2 whitespace-nowrap border-l border-gray-200">
                      <div className="flex items-center space-x-1">
                        ...
                        <Button 
                          ...
                          variant="ghost" 
                          className={`h-7 w-7 p-0 ${
                            alertStatuses[order.id] || order.alertStatus
