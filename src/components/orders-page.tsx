...
export function OrdersPage({ searchTerm = '', onSearchChange, onNavigateToOrders, showAddOrderModal = false, setShowAddOrderModal }: OrdersPageProps) {
  ...
  }, [effectiveSearchTerm, showUrgentOnly, assemblyLineFilter, gmsoaFilter, partyFilter, dateFilterMode, dateFrom, dateTo]);
  ...
  const handlePrint = () => {
    try {
      ...
      import('jspdf').then(({ default: jsPDF }) => {
        ...
        pdf.text('Orders Management Report', 20, 20);
  ...
  return (
    <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in bg-white min-h-screen">
      <div className="mb-8">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex flex-col sm:flex-row gap-4 lg:items-center">
              ...
              <Button 
                ...
                className="flex items-center gap-2 bg-white/80 backdrop-blur-sm border-gray-200/60 hover:bg-[#e8f0f9] hover:border-[#a3c4e7] disabled:opacity-50 disabled:cursor-not-allowed"
              >
        ...
        {effectiveSearchTerm && (
          <div className="mt-4 p-3 bg-[#e8f0f9]/80 backdrop-blur-sm rounded-lg border border-[#a3c4e7]/60">
            <p className="text-sm text-[#123a7f]">
              {filteredOrders.length} {filteredOrders.length === 1 ? 'order' : 'orders'} found for "{effectiveSearchTerm}"
              {filteredOrders.length === 0 && (
                <span className="ml-2 text-[#174a9f]">
            ...
            <div className="space-y-4 border-t pt-4">
              <div className="flex items-center space-x-2">
                <Checkbox 
                  ...
                  onCheckedChange={(checked) => {
                    setSplitOrder(checked as boolean);
                    if (!checked) {
                      setSplitAssignStep('');
                      setSplitAssignQty('');
                      setQuickAssignErrors(prev => {
                        const newErrors = {...prev};
                        delete newErrors.splitAssignStep;
                        delete newErrors.splitAssignQty;
                        delete newErrors.sameEngineer;
                        delete newErrors.totalQtyMismatch;
                        delete newErrors.totalQty;
                        return newErrors;
                      });
              ...
              {splitOrder && (
                <div className="space-y-4 pl-6 border-l-2 border-blue-200">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <Label htmlFor="splitAssignStep">Second Workflow Step</Label>
                      <Select 
                        value={splitAssignStep} 
                        onValueChange={(value) => {
                          setSplitAssignStep(value);
                          if (quickAssignErrors.splitAssignStep || quickAssignErrors.sameEngineer) {
                            setQuickAssignErrors(prev => {
                              const newErrors = {...prev};
                              delete newErrors.splitAssignStep;
                              delete newErrors.sameEngineer;
                      ...
                      >
                        <SelectTrigger id="splitAssignStep" className={quickAssignErrors.splitAssignStep ? 'border-red-500' : ''}>
                          <SelectValue placeholder="Select step" />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="material-issue">Material Issue</SelectItem>
                      ...
                      {quickAssignErrors.splitAssignStep && (
                        <p className="text-sm text-red-500">{quickAssignErrors.splitAssignStep}</p>
                      )}
                    </div>
                    
                    <div className="space-y-2">
                      <Label htmlFor="splitAssignQty">Split Quantity</Label>
                      <Input 
                        id="splitAssignQty" 
                        type="number" 
                        value={splitAssignQty}
                        onChange={(e) => {
                          setSplitAssignQty(e.target.value);
                          if (quickAssignErrors.splitAssignQty) {
                            setQuickAssignErrors(prev => {
                              const newErrors = {...prev};
                              delete newErrors.splitAssignQty;
                              delete newErrors.totalQtyMismatch;
                        ...
                        max={selectedOrder?.qtyPending}
                        className={quickAssignErrors.splitAssignQty ? 'border-red-500' : ''}
                      />
                      {quickAssignErrors.splitAssignQty && (
                        <p className="text-sm text-red-500">{quickAssignErrors.splitAssignQty}</p>
                      )}
                    </div>
                  </div>
                  ...
                  {quickAssignErrors.totalQty && (
                    <div className="p-3 bg-red-50 border border-red-200 rounded-lg">
                      <p className="text-sm text-red-600">{quickAssignErrors.totalQty}</p>
                    </div>
                  )}

                  {/* Summary */}
                  {quickAssignQty && splitAssignQty && (
                    <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                      <p className="text-sm text-blue-700">
                        Total: {Number(quickAssignQty) + Number(splitAssignQty)} of {selectedOrder?.qtyPending} units
                      </p>
                    </div>
                  )}
                </div>
